...
import itertools
import uuid
from dynamo_db_rough import log_event  # adjust the filename if your logging code is saved as voa_logging.py
...
class Cart:
    def __init__(self, session_id=None):
         ...
         self.session_id = session_id or str(uuid.uuid4())  # generate new if none provided
    ...
    def add(self, li: LineItem):
        ...
        for e in self.items:
            if (e.item, e.size_or_variant, tuple(e.mods)) == (li.item, li.size_or_variant, tuple(li.mods)):
                e.qty += li.qty
                log_event(self.session_id, "OrderEngine", "ItemMerged", f"{li.qty}x {li.item}")
                return
        self.items.append(li)
        log_event(self.session_id, "OrderEngine", "ItemAdded", li.label())


    def remove_last(self) -> bool:
        if not self.items:
            log_event(self.session_id, "OrderEngine", "RemoveFailed", "Cart empty")
            return False
        removed = self.items.pop()
        log_event(self.session_id, "OrderEngine", "ItemRemoved", removed.label())
        return True
    ...
    def apply_edit(self, edits: List[dict]) -> List[str]:
        ...
        for cmd in edits:
            ...
            for li in targets:
                ...
                log_event(self.session_id, "OrderEngine", "ItemEdited", li.label())
        return changed
    ...
    def total(self) -> float:
        total_value = round(self.subtotal() + self.tax(), 2)
        log_event(self.session_id, "OrderEngine", "OrderTotalCalculated", f"{total_value}")
        return total_value


    def summary_speech(self) -> str:
        if not self.items:
            log_event(self.session_id, "OrderEngine", "SummaryRequested", "Cart empty")
            return "Your cart is empty."
        total = self.total()
        log_event(self.session_id, "OrderEngine", "SummaryRequested", f"Total ${total}")
        return f"That will be ${total:.2f}. Would you like anything else?"

if __name__ == "__main__":
    c = Cart()
    item1 = LineItem(item="cheeseburger", qty=2)
    c.add(item1)
    c.add(LineItem(item="fries", qty=1))
    c.remove_last()
    c.summary_speech()
